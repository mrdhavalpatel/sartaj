...
function Index2() {
  ...
  // Memoize URL modification handler
  const handleURLModification = useCallback(() => {
    const modifiedURLEng = replaceBaseURL(currentURL);
    const modifiedURLJp = replaceBaseURL(currentURL, "jp");
  }, [currentURL]);
  ...
  useEffect(() => {
    ...
    const routeChangeComplete = () => handleURLModification();
    router.events.on("routeChangeComplete", routeChangeComplete);

    return () => {
      router.events.off("routeChangeComplete", routeChangeComplete);
    };
  }, [handleURLModification, router.events]);
  ...
  // Handle English URL formatting
  useEffect(() => {
    if (locale === "eng") {
      const engIndex = window?.location?.href?.indexOf("/eng");
      if (engIndex !== -1) {
        const urlAfterEng = window.location.href.substring(engIndex + 4);
        let modifiedUrl = window.location.origin + "/eng" + urlAfterEng.replace(/\/+/g, "/");
        modifiedUrl = modifiedUrl.replace(/\/$/, "");
  ...
  // Memoize section title components
