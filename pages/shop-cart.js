...
const Cart = ({
  ...
  const deleteProductFromCart = async (id) => {
    const response = await axios
      .delete(`${API_BASE_URL}customer/cart/remove-to-cart/${id}`, {
        headers: {
  ...
  const getCartsItem = async (token) => {
    const response = await axios
      ...
      .then((response) => {
        setCartItem(response?.data?.cartProducts);
      })
      .catch((error) => {
        console.log("error", error?.code === "ERR_NETWORK");
      });
  };
  const filterProductArray = (payload) => {
    const updatedArray = payload.map((product, index) => {
      return {
        product_id: index + 1,
        qty: product.quantity + 1, // increase quantity by 1
      };
    });
    return updatedArray;
  };
  const addCurrentItemsInCart = async (token) => {
    let payload = localStorage.getItem("dokani_cart");

    const response = await axios
      .post(
        `${API_BASE_URL}customer/cart/add-items`,
        { cart: filterProductArray(JSON.parse(payload)) },
        {
          headers: {
            "Access-Control-Allow-Origin": "*",
            "Content-Type": "application/json",
                      ...
                      <tbody>
                        {cartItem?.map((item, i) => (
                          <tr key={i}>
                            ...
                            <td className="action" data-title="Remove">
                              <a
                                onClick={(e) => deleteProductFromCart(item.id)}
                                className="text-muted"
                              >
                                <i className="fi-rs-trash"></i>
                              </a>
