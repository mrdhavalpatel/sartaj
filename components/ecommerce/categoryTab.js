...
function CategoryTab({ active, catAll, cat1, cat2, cat3 }) {
  return (
    <>
      <div className="section-title style-2 wow animate__animated animate__fadeIn">
        <h3>Popular Products</h3>
        <ul className="nav nav-tabs links" id="myTab" role="tablist">
          <li className="nav-item" role="presentation">
            <button className={active === "1" ? "nav-link active" : "nav-link"}>
              All
            </button>
          </li>
          <li className="nav-item" role="presentation">
...
export async function getStaticProps() {
  const server = process.env.SERVER_API_URL;
  const [catAll, cat1, cat2, cat3] = await Promise.all([
    fetch(`${server}/static/product.json`).then((res) => res.json()),
    fetch(`${server}/static/product.json`).then((res) => res.json()),
    fetch(`${server}/static/product.json`).then((res) => res.json()),
    fetch(`${server}/static/product.json`).then((res) => res.json()),
    // latest = new
  ...
  return {
    props: { active: "1", catAll, cat1, cat2, cat3 },
  };
}
